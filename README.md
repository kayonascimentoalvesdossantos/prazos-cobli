...
import { Search, MapPin, Filter, X, Globe, Truck, Clock, AlertCircle, ChevronDown, Package } from 'lucide-react';

/**
 * Base de Dados Integral - Cobli
 * Formato TSV: CIDADE	UF	TIPO	REGIÃO	PRAZO
 */
...
const parseData = (raw: string): ShippingInfo[] => {
  const lines = raw.split('\n');
  const result: ShippingInfo[] = [];

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i].trim();
    if (!line || line.toUpperCase().startsWith('CIDADE')) continue;
    // O novo formato usa TAB (\t) como separador
    ...
    result.push({
      ...
      tipo: parts[2]?.trim() || '',
      regiao: parts[3]?.trim() || '',
      prazo: parts[4]?.trim() || 'Sob Consulta'
    });
  }
  return result;
};
...
const App = () => {
  ...
  const ufs = useMemo(() => Array.from(new Set(data.map((item) => item.uf))).sort(), [data]);
  const regioes = useMemo(() => Array.from(new Set(data.map((item) => item.regiao))).filter(r => r).sort(), [data]);

  const filteredData = useMemo(() => {
    return data.filter((item) => {
      const cityNormalized = item.cidade.toLowerCase();
      const ufNormalized = item.uf.toLowerCase();
      const searchNormalized = searchTerm.toLowerCase();
      const matchesSearch = cityNormalized.includes(searchNormalized) || ufNormalized.includes(searchNormalized);
      const matchesUf = selectedUf === '' || item.uf === selectedUf;
      const matchesRegiao = selectedRegiao === '' || item.regiao === selectedRegiao;
      return matchesSearch && matchesUf && matchesRegiao;
    });
  }, [data, searchTerm, selectedUf, selectedRegiao]);
  ...
  const loadMore = () => setLimit(prev => prev + 200);
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      <header className="relative h-[340px] flex items-center justify-center overflow-hidden">
        ...
        >
          <div className="absolute inset-0 bg-gradient-to-tr from-blue-950/90 via-slate-900/85 to-slate-900/80 mix-blend-multiply"></div>
        </div>

        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-xl px-4 py-2 rounded-full border border-white/20 mb-8 shadow-2xl">
            <Package className="w-4 h-4 text-blue-400" />
            <span className="text-white text-[10px] font-black uppercase tracking-[0.25em]">Cobli • Logística</span>
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tighter drop-shadow-2xl mb-6">
            Prazos de Entrega
          </h1>
          <p className="text-blue-100 text-sm md:text-lg font-medium max-w-2xl mx-auto opacity-95 leading-relaxed">
            Consulte o tempo estimado de transporte para todo o Brasil.
          </p>
        </div>
      </header>

      {/* Painel de Filtros */}
      <main className="max-w-6xl mx-auto px-4 -mt-16 pb-24 relative z-20">
        <div className="bg-white rounded-[2.5rem] shadow-[0_25px_50px_-12px_rgba(0,0,0,0.15)] p-6 md:p-10 mb-10 border border-slate-200">
          <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-end">
            
            <div className="md:col-span-5 space-y-3">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Localizar Destino</label>
              <div className="relative group">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-10 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 transition-all placeholder:text-slate-400 font-semibold"
                  value={searchTerm}
                  onChange={(e) => { setSearchTerm(e.target.value); setLimit(100); }}
                />
                {searchTerm && (
                  <button onClick={() => setSearchTerm('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-300 hover:text-slate-600 p-1">
                    <X className="w-5 h-5" />
                  </button>
                )}
              </div>
            ...
            <div className="md:col-span-3 space-y-3">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Região Geográfica</label>
              <div className="relative">
                <Globe className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                <select
                  className="w-full pl-12 pr-4 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 appearance-none cursor-pointer font-bold text-slate-700"
                  value={selectedRegiao}
                  onChange={(e) => { setSelectedRegiao(e.target.value); setLimit(100); }}
                >
            ...
            <div className="md:col-span-2 space-y-3">
              <label className="text-[11px] font-black text-slate-500 uppercase tracking-widest ml-1">Estado</label>
              <div className="relative">
                <Filter className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 w-5 h-5" />
                <select
                  className="w-full pl-12 pr-4 py-5 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:ring-4 focus:ring-blue-500/10 focus:border-blue-600 appearance-none cursor-pointer font-bold text-slate-700"
                  value={selectedUf}
                  onChange={(e) => { setSelectedUf(e.target.value); setLimit(100); }}
                >
            ...
            <div className="md:col-span-2">
              <button
                ...
                className="w-full py-5 text-white bg-blue-600 hover:bg-blue-700 rounded-2xl font-black transition-all shadow-xl active:scale-[0.97] uppercase tracking-widest text-[11px]"
              >
                Limpar
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="bg-white border border-slate-200 rounded-3xl p-6 flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                    <h4 className="font-black text-xs uppercase text-slate-400 tracking-widest mb-1">Destinos Listados</h4>
                    <p className="text-2xl font-black text-slate-900 leading-none">{filteredData.length.toLocaleString('pt-BR')}</p>
                </div>
            </div>
             <div className="bg-white border border-slate-200 rounded-3xl p-6 flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                    <h4 className="font-black text-xs uppercase text-slate-400 tracking-widest mb-1">Operação</h4>
                    <p className="text-2xl font-black text-slate-900 leading-none">Prazos</p>
                </div>
            </div>
             <div className="bg-white border border-slate-200 rounded-3xl p-6 flex items-center gap-5 shadow-sm hover:shadow-md transition-shadow">
                ...
                <div>
                    <h4 className="font-black text-xs uppercase text-slate-400 tracking-widest mb-1">Abrangência</h4>
                    <p className="text-2xl font-black text-slate-900 leading-none">Nacional</p>
                </div>
            </div>
        </div>
        ...
        <div className="bg-white rounded-[2.5rem] shadow-xl border border-slate-200 overflow-hidden">
          {filteredData.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="w-full text-left">
                <thead className="bg-slate-50/50 border-b border-slate-200">
                  <tr>
                    <th className="px-10 py-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.25em]">Cidade / Tipo</th>
                    <th className="px-10 py-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.25em]">Localização</th>
                    <th className="px-10 py-8 text-[11px] font-black text-slate-400 uppercase tracking-[0.25em] text-right">Prazo Estimado</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {filteredData.slice(0, limit).map((item, index) => (
                    <tr key={`${item.uf}-${item.cidade}-${index}`} className="hover:bg-blue-50/40 transition-all group">
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-5">
                          <div className="w-14 h-14 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white group-hover:-rotate-3 transition-all duration-300 shadow-sm">
                            <MapPin className="w-6 h-6" />
                          </div>
                          <div>
                            <span className="font-black text-slate-900 text-2xl leading-none tracking-tighter block mb-2 uppercase">
                              {item.cidade}
                            </span>
                            <span className="px-2.5 py-1 bg-slate-100 text-slate-500 rounded-lg text-[9px] font-black uppercase tracking-widest">{item.tipo}</span>
                          </div>
                        </div>
                      </td>
                      <td className="px-10 py-8">
                        <div className="flex flex-col gap-2">
                          <span className="inline-flex items-center w-fit px-4 py-1.5 bg-slate-900 text-white font-black rounded-xl text-xs tracking-widest">
                            {item.uf}
                          </span>
                          <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">{item.regiao}</span>
                        </div>
                      </td>
                      <td className="px-10 py-8 text-right">
                        {item.prazo.toLowerCase().includes('consulta') ? (
                          <span className="px-6 py-3 bg-amber-50 text-amber-700 rounded-2xl text-[11px] font-black uppercase tracking-[0.15em] border border-amber-200/50 shadow-sm">
                            Sob Consulta
                          </span>
                        ) : (
                          <div className="flex flex-col items-end">
                            <span className="inline-flex items-center gap-3 px-8 py-4 bg-blue-50 text-blue-700 rounded-[1.5rem] text-3xl font-black border border-blue-100 group-hover:shadow-lg group-hover:-translate-y-1 transition-all duration-300">
                              <Clock className="w-6 h-6" />
                              {item.prazo} {parseInt(item.prazo) === 1 ? 'Dia' : 'Dias'}
                            </span>
                            <span className="text-[10px] font-black text-slate-300 mt-3 uppercase tracking-[0.25em]">Após Coleta</span>
                          </div>
                        )}
                      </td>
                    </tr>
              ...
              {/* Botão Carregar Mais */}
              <div className="p-12 bg-slate-50/40 text-center border-t border-slate-200">
                {limit < filteredData.length ? (
                  <div className="space-y-6">
                    <p className="text-slate-400 font-bold text-xs uppercase tracking-[0.2em]">
                      Exibindo {limit} de {filteredData.length.toLocaleString('pt-BR')} resultados
                    </p>
                    <button
                      onClick={loadMore}
                      className="inline-flex items-center gap-4 bg-white hover:bg-blue-600 hover:text-white text-blue-600 border-2 border-blue-100 px-10 py-5 rounded-[2rem] font-black transition-all shadow-xl active:scale-95 group uppercase tracking-widest text-[11px]"
                    >
            ...
            <div className="py-40 flex flex-col items-center justify-center text-center px-10">
              <div className="w-32 h-32 bg-slate-50 rounded-full flex items-center justify-center mb-10 ring-[12px] ring-white shadow-inner">
                <Search className="w-12 h-12 text-slate-200" />
              </div>
              <h3 className="text-3xl font-black text-slate-900 mb-4 tracking-tighter">Destino não localizado</h3>
              <p className="text-slate-400 max-w-md mx-auto text-base font-medium leading-relaxed mb-12">
                Não encontramos resultados para "{searchTerm}". Tente pesquisar apenas pela UF ou confira a grafia da cidade.
              </p>
              <button onClick={clearFilters} className="bg-slate-900 hover:bg-blue-600 text-white px-12 py-6 rounded-3xl font-black transition-all shadow-2xl uppercase tracking-widest text-xs active:scale-95">
                Redefinir Filtros
              </button>
            </div>
          )}
      ...
      <footer className="py-24 border-t border-slate-200 bg-white">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 items-center gap-16">
          <div className="flex items-center gap-6">
            <div className="w-16 h-16 bg-blue-600 rounded-[1.5rem] flex items-center justify-center shadow-2xl shadow-blue-200 p-4 rotate-3">
               <Truck className="w-full h-full text-white" />
            </div>
            <div>
              <p className="font-black text-slate-900 text-4xl leading-none uppercase tracking-tighter">COBLI</p>
              <p className="text-[11px] font-black text-blue-500 uppercase tracking-[0.45em] mt-3">Melhore a gestão da sua operação</p>
            </div>
          </div>
          <div className="text-center md:text-right space-y-4">
              <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest leading-relaxed">
                Libertamos a logística através da tecnologia.
              </p>
              <p className="text-slate-400 text-[10px] font-medium leading-relaxed italic opacity-80">
                Prazos calculados em dias úteis a partir da coleta. Sujeito a variações climáticas e operacionais.
              </p>
          </div>
        </div>
      </footer>
